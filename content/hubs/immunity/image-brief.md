...
**1. Slug (ASCII, language-neutral):** `immunity`
File names: `immunity-preview` · `immunity-hero`.

**2. Article title (EN) + one-line summary:** "Microbes and immunity: how tolerance is taught, not born" — the immune system is built to live with microbes; bacteria on the gut lining teach macrophages and regulatory T cells which signals mean danger and which mean neighbour, and when that learning fails the result is chronic inflammation.
...
- Beneath the lining: loose, soft tissue threaded with sparse fine fibres.
- Bacteria: the hub names no species for this story, so they are generic — short rods with rounded ends and small round cocci.
- Regulatory T cells (second frame): small round cells, much smaller than the macrophage, mostly filled by one large round nucleus, the surface lightly textured with very short projections.
- Short-chain fatty acids (second frame): tiny molecules, each a small cluster of a few linked beads.
...
- en preview alt: "Illustration: a macrophage under the gut lining curls a coral arm around one rod-shaped bacterium in a warm spot of light"
...
- Macrophages, the one-cell-thick lining, short-chain fatty acids and regulatory T cells are all named in the hub.
- The second frame shows the calm path only; the alarm path from cell-wall fragments is left out so the eye walks one line, not denied.
- Nothing suggests that any strain or product boosts immunity; the hub calls that the wrong verb.
...
- uk preview alt: "Ілюстрація: макрофаг під слизовою кишківника обіймає кораловим відростком одну паличкоподібну бактерію в теплій плямі світла"
- uk hero alt: "Ілюстрація: бактерії над слизовою кишківника завтовшки в одну клітину виділяють дрібні молекули; макрофаг під нею вбирає їх і лишається спокійним поруч із золотими регуляторними Т-клітинами"
