...
#        or WRANGLER_OAUTH=1 to use the machine's own `wrangler login` and read no token at all.
set -euo pipefail
OUT="${1:?out dir}"
...
node -e "for (const r of require(process.argv[1])) console.log(r.key + '\t' + r.file)" "$PLAN" | while IFS=$'\t' read -r key file; do
  ...
  npx --yes wrangler@4 r2 object put "dasexperten-images/$key" --file="$file" --content-type="$ct" --remote >/dev/null 2>&1 && echo "ok  $key" || echo "ERR $key"
done
