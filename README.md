...
**Owner brief (2026-09-02):** portal first on Cloudflare, then the domain; 18 languages (the 16 of dasexperten.com + Japanese + Korean); two generated images per topic — a *preview* (macro, "zoomed", Lisa) and a *hero* (Otto's 3D scientific infographic, no people — Owner 2026-09-16); SEO/GEO wording with Jurgen and Julian; later phases: consulting (Lauda · Roberta · Alexandra) and a products section.
...
| [`docs/IMAGE_PROMPT.md`](./docs/IMAGE_PROMPT.md) | The one-paragraph image prompt, filled per article — 3D infographics from the article text; hero by Otto, preview by Lisa, platform-native engine, checked by Marika |
...
- **Images:** Magnus describes in plain words → Marika names slot, ratio and focus → Otto (hero, 3D infographic) and Lisa (preview, macro world) write their own engine words and generate on the platform-native engine, not Higgsfield → Marika accepts twice. No people, no text, no product in any frame (§4, §4d, §4e-1a, §4f).
